...
current_character = None # Holds the active character dictionary
all_quests = {}          # Dictionary of all quests in the game
all_items = {}           # Dictionary of all items in the game
game_running = False     # Flag to control the main game loop
...
def new_game():
    ...
    try:
        # Create a new character using the character manager
        char = character_manager.create_character(name, cls)
    except InvalidCharacterClassError as e:
        print(f"Invalid class: {e}")
        return

    # Initialize equipped items and link item data
    ...
    # Attempt to save the newly created character
    try:
...
def load_game():
    ...
    print("Saved characters:") # Display all saved characters
    for idx, s in enumerate(saves, start=1):
        print(f"{idx}) {s}")

    while True: # Prompt for character selection
...
def view_character_stats():
    ...
    try:
        ...
        # Fallback if quest_handler fails
