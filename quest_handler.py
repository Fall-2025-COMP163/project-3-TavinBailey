...
def accept_quest(character, quest_id, quest_data_dict):
    ...
    if quest_id not in quest_data_dict: # Check if the quest exists in the quest dictionary
        raise QuestNotFoundError(f"Quest '{quest_id}' does not exist.")

    quest = quest_data_dict[quest_id]

    if quest_id in character["completed_quests"]: # Check if the quest is already completed
        raise QuestAlreadyCompletedError(f"Quest '{quest_id}' already completed.")

    if quest_id in character["active_quests"]: # Check if the quest is already active
        raise QuestRequirementsNotMetError(f"Quest '{quest_id}' already active.")

    if character.get("level", 1) < quest["required_level"]: # Check if the character meets the level requirement
        raise InsufficientLevelError(
            f"Level {quest['required_level']} required to accept quest '{quest_id}'."
        )

    prereq = quest["prerequisite"] # Check prerequisite quest completion
    ...
    # All checks passed; add quest to active quests
...
def complete_quest(character, quest_id, quest_data_dict):
    ...
    if quest_id not in quest_data_dict: # Check if quest exists
        raise QuestNotFoundError(f"Quest '{quest_id}' does not exist.")

    if quest_id not in character["active_quests"]: # Ensure quest is active
...
def get_available_quests(character, quest_data_dict):
    ...
    for qid, quest in quest_data_dict.items():
        # Skip quests that are completed or active
        ...
        # Skip quests where level requirement not met
        if character["level"] < quest["required_level"]:
            continue

        # Skip quests where prerequisite not completed
...
if __name__ == "__main__":
    ...
    # }
    #
    # Attempt to accept the sample quest
    try:
