...
def add_item_to_inventory(character, item_id):
    ...
    if len(inventory) >= MAX_INVENTORY_SIZE:
        raise InventoryFullError("Inventory is full.") # Raise error if inventory is full

    inventory.append(item_id)
    return True

def remove_item_from_inventory(character, item_id): # Remove item from inventory, raise error if not found
    """
...
def has_item(character, item_id): # Check if character has a specific item
    """
...
def count_item(character, item_id): # Count how many of an item the character has
    """
...
def get_inventory_space_remaining(character): # Calculate remaining inventory slots
    """
...
def clear_inventory(character): # Clear all items from inventory
    """
...
def use_item(character, item_id, item_data): # Use a consumable item, apply its effect, remove from inventory
...
def equip_weapon(character, item_id, item_data): # Equip a weapon, apply its effect, unequip old weapon if exists
    """
    Equip a weapon
    ...
    if character.get("equipped_weapon"):
        ...
        apply_stat_effect(character, stat, -value) # remove old bonus
...
def equip_armor(character, item_id, item_data): # Equip armor, similar logic as equip_weapon
    """
    Equip armor
...
def unequip_weapon(character): # Unequip weapon, remove bonus, return to inventory
    """
...
def unequip_armor(character): # Unequip armor, remove bonus, return to inventory
    """
...
def purchase_item(character, item_id, item_data): # Purchase an item, subtract gold, add to inventory
    """
...
def parse_item_effect(effect_string): # Parse item effect string like "health:20"
    """
...
def apply_stat_effect(character, stat_name, value): # Apply a stat change to character
    """
...
def display_inventory(character, item_data_dict): # Display inventory with item counts
    """
